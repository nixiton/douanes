



<div class="box dark">
        
        <div  class="body">

        <form class="form-horizontal">

            <div class="form-group">
                <label for="text1" class="control-label col-lg-4">Libellé  </label>                                                                                                                                                                                                                         </label>

                    <div class="col-lg-4">
                        <input type="text" id="text1" placeholder="" class="form-control">
                    </div>   

                    <div class="col-lg-4">
                        
                    </div>
                    
                </div>

            <div class="form-group">
                            <label class="control-label col-lg-4">Nomenclature</label>

                            <div class="col-lg-4">
                                <select class="form-control">
                                    <option>001</option>
                                </select>
                            </div>

                            <div class="col-lg-1">
                                <a href="javascript:;" id="add_type_mat" class="btn btn-default add_list"><i class="fa fa-plus"></i></a>
                            </div>

                            <div class="col-lg-3">
                                
                            </div>

                        </div>

            <div class="form-group">
                <label class="control-label col-lg-4">Caractéristiques</label>

                <div class="col-lg-4">
                    <textarea id="text4" class="form-control"></textarea>
                </div>

                <div class="col-lg-4">
                    
                </div>

            </div>

            <div class="form-group">
                  <label for="text1" class="control-label col-lg-4"></label>
                
                <div class="col-lg-4">
                    <input class="btn btn-lg btn-primary btn-block"  type="submit" value="Créer"/>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              
                </div>

                <div class="col-lg-4"></div>
                
            </div>

        </form>

    </div>

</div>